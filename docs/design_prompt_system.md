...
- **Re-rolls draw contract vs cap**: The "re-rolls never refuse" contract in `PromptDecks.drawOneExcluding` applies to the draw algorithm itself (it relaxes rather than throwing when unseen prompts run dry). The *callable* `rerollPrompt` now enforces the 3-per-round cap.
