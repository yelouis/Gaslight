...
- **Sealed Storage (`/rooms/{roomCode}/sealed/{cardId}`)**: The list of prompts a player has seen and rejected during re-rolls (`seenPrompts: string[]`), re-rolls used this round (`rerollsThisRound: number`), and distinct prompts drawn this round (`rerollCandidates: string[]`) are stored in the server-only `/sealed/{cardId}` subcollection (keyed by target player ID), preserving player privacy by keeping declined prompts and candidate state off the public, client-readable room document.
- **Lazy Seeding & Accumulation**: `rerollPrompt` reads the player's sealed document inside the transaction prior to any writes, seeding `seenPrompts` lazily from `targetCard.promptText` if not yet created. Each successful re-roll increments `rerollsThisRound` and appends the new prompt to `seenPrompts` and (if distinct) `rerollCandidates`.
- **Draw History Exclusion (Trap Fixed Wave AC4, September 2026)**: Previously, re-rolls excluded only prompts currently live on table cards (`inPlay`), ignoring `seenPrompts` (`cardSeen`). This allowed prompts a player just rerolled away from to immediately reappear in the same round. In Wave AC4, both catalogue and custom-pool draw branches strictly exclude player history:
  - Catalogue deck branch: `PromptDecks.drawOneExcluding(deckId, new Set([...inPlay, ...cardSeen]), inPlay)`.
  - Custom-pool branch: filters candidates against both `cardSeen` and `inPlay`.
  - History is passed in `excluded` (soft exclusion), while `inPlay` is passed in `mustAvoid` (hard exclusion), ensuring the draw never refuses on a small deck while two players never share a prompt.
- **Per-Round Re-Roll Cap (`kMaxRerollsPerRound = 3`)**: Re-rolls are capped at 3 per round per player. `rerollPrompt` rejects with `failed-precondition` when `rerollsThisRound >= kMaxRerollsPerRound`. Both client and server reference this shared constant. `concludeResolutionRound` explicitly resets `rerollsThisRound: 0` and `rerollCandidates: []` on document replacement.
- **Prompt Chooser (`selectRerolledPrompt`)**: Once all 3 re-rolls are spent (`rerollsThisRound >= 3`), the player can select any candidate from `rerollCandidates` to answer. The callable validates:
  1. `room.currentPhase === "truth"` (else `failed-precondition`).
  2. Caller owns the card.
  3. `rerollsThisRound >= kMaxRerollsPerRound` (else `failed-precondition`).
  4. `promptText` is in `rerollCandidates` (else `invalid-argument`).
  5. `promptText` is not currently in play on any card (re-validated at selection time; else `failed-precondition` collision error).
  6. Player has not already submitted a truth answer this round.
  On collision (`failed-precondition`), the client removes the collided candidate and displays `"Prompt was chosen by another player."`.
- **Degradation on Small Decks**: On small decks where unseen prompts run out, `rerollCandidates` deduplicates on append so the chooser never displays the same prompt twice.
- **Fallback Deck Top-Up on Exhaustion (Issue 174 / Wave AC5 & AD2, September 2026)**: In small decks (25 prompts), high re-roll consumption can exhaust unseen prompts inside a single round. Rather than silently degrading into repeated prompts:
  - `PromptDecks.drawWithFallbackExcluding(deckId, excluded, mustAvoid)` attempts to draw an unseen prompt outside `excluded` from `deckId`.
  - **Exhaustion-only ordering**: While `deckId` still has unseen prompts for this player, the fallback is **never consulted** (`AC5.4`). It reaches into the fallback deck (`PromptDecks.getFallbackDeckId()`) only when the room deck's unseen pool is completely exhausted (`AC5`).
  - **Applied to both re-rolls and round deals**: Both `rerollPrompt` and `concludeResolutionRound` use `drawWithFallbackExcluding`, protecting dealt prompts as well as re-roll draws.
  - **Content Rating Safety Property (load-bearing)**: The fallback deck (`hypotheticals`) is strictly rated **PG** and is the only deck marked `isFallback`. Because the fallback is PG, topping up can only make content milder, never more explicit, ensuring family-friendly rooms never receive rated content. This PG rating property is load-bearing (`AC5.5`), asserted by unit tests (`functions/test/prompt_decks.spec.ts`).
  - **Terminal Case**: When history covers both the room deck and the fallback deck, the draw relaxes to `mustAvoid` (prompts live on cards) and never refuses (`AC5.3`).
