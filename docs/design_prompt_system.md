...
- **Fallback Deck Top-Up on Exhaustion (Issue 174 / Wave AC5, September 2026)**: In small decks (25 prompts), high re-roll consumption can exhaust unseen prompts inside a single round. Rather than silently degrading into repeated prompts:
  ...
  - If no unseen prompts remain in `deckId`, it reaches into the fallback deck (`PromptDecks.getFallbackDeckId()`) to draw an unseen prompt from the fallback before relaxing.
  ...
  - **Content Rating Safety Property**: The fallback deck (`hypotheticals`) is strictly rated **PG** and is the only deck marked `isFallback`. Topping up can only make content milder, never more explicit, ensuring family-friendly rooms never receive rated content. Asserted by unit tests (`functions/test/prompt_decks.spec.ts`).
  - **Terminal Case**: When history covers both the room deck and the fallback deck, the draw relaxes to `mustAvoid` (prompts live on cards) and never refuses.
