# Agent Execution Guide — Queue Complete: no approved work — September 11, 2026

**You are an engineering agent with no memory of this project.**

**Every number and literal string in this document is a decision, not a suggestion.**

**Wave AA is delivered and independently verified.** All sixteen items (Issues 153–164, 166–169) shipped, were read in source rather than taken from commit bodies, and their guards were re-falsified this session. There is **no approved queue**.

**Two things are open and neither is yours to start:**

- **Issue 170** — a scoring balance interaction found during verification — is filed in `ongoing_general_errors.md` with three options and a **blank** `Your selection: _____`. **An unselected issue is a question, not an instruction.** Never fill that line in, and never treat `(recommended)` as approval.
- **Issue 165** (Quiplash differentiation) is **deferred at the user's direction** and stays open. Do not work on it, do not close it, and do not let a consolidation drop it.

**⚠️ There is one real action outstanding, and it is in §2.1: the functions are not deployed.**

**Do not invent work.** The only legitimate actions are in §2.1.

---

## 1. Verified baseline — measured this session on `9e82126`

Every number below was run bare in this session. **This is the regression bar.**
...
| `./scripts/check_decks_in_sync.sh` | **exit 0** — 5 decks, 918 lines compared |
| `./scripts/check_playthrough_evidence.sh` (all four invocations) | **exit 0** — 20 PASS, 1 NOT RUN, 0 FAIL |
| `./scripts/check_deploy_fresh.sh` | **exit 1 — STALE.** ⚠️ See §2.1. Not a regression; the functions simply have not been deployed since Wave AA. |

**⚠️ `flutter analyze lib test` exits 1 even when clean** — it exits non-zero on *infos*. **The bar is `0 errors` and `0 warnings`, not `exit 0`.** Use `lib test`, never bare `flutter analyze`.

**⚠️ Infos are now 195, down from 206.** Wave AA deleted `dealt_card_overlay.dart` and its two test files. **The bar is 195 and no new infos.**

**⚠️ Read exit codes bare, never through a pipe.** `flutter analyze … | tail` reports `tail`'s status, which is always 0. This trap was hit again this session and caught only by re-running bare.

**⚠️ `pubspec.yaml` is at `1.0.0+7` and build 7 has NOT been uploaded.** TestFlight builds 5 and 6 are live; **6 carries the Issue 152 leave bug**, so once 7 ships, expire both. `flutter build ipa` fails at the *export* step on this machine (no Apple Distribution certificate) — **expected, not a build failure**. Verify the **archive's** timestamp and `CFBundleVersion`, never the `.ipa`. Full detail in `README.md` → Releasing.
...
(1) A selection exists? If NO -- stop. Never fill in a `Your selection:` line.
    Issue 170 is filed and UNSELECTED. Issue 165 is deferred. Neither is work.
(2) Read exit codes BARE. `flutter analyze ... | tail` reports tail's status,
    which is always 0. This trap was hit again during the September 11
    verification and caught only by re-running without the pipe.
(3) Changing scoring? Change BOTH functions/src/scoring_logic.ts AND the
    test-only mirror lib/utils/scoring_logic.dart, and re-run the sum invariant.
(4) Adding a callable? Copy castVote's authorization shape (index.ts:880):
...
(5) WRITE the falsifying validation. Run it. OBSERVE IT FAIL. Record the exact
    output in the commit body. A DELETION STILL NEEDS ONE.
(6) VALIDATE, including every over-reach guard, then RE-RUN THE GUARD WITH THE
    FIX REMOVED and confirm it fails.
(7) State bugs: the test must NOT re-pump the widget between steps. Re-pumping
    builds a fresh State and passes against broken code -- that is how Issue
    152 shipped. Three Wave AA items carried this trap and all three are
    tested with a single pumpWidget.
(8) ENUMERATE EVERY INVOCATION of what you changed. pendingScoreDeltas and
    pendingScoreBreakdown have THREE flush sites.
(9) Changing two things that write to the SAME number, document or screen
    region? Compute the COMBINED worst case before calling it done, as a table
    of real figures. Per-item review does not see interactions -- lesson 2.41.
(10) RE-RUN THE FULL BATTERY -- bare, except flutter analyze, where the bar is
     0 errors / 0 warnings / 195 infos and the code is always 1.
(11) SHIPPING? DEPLOY FUNCTIONS FIRST (section 2.1), re-apply CLEANUP_DRY_RUN
     =false and read it back, then follow README -> Releasing. Next build is
     1.0.0+7; the CLI export failure is expected, distribute the .xcarchive.
(12) COMMIT: one item, one Conventional Commit, WHY in the body. Move the issue
     to the SINGLE existing Resolved heading, leave ONE line there, and put the
     durable consequence in the design doc.
```

**The queue is empty. Do not invent work.** The only legitimate actions are in §2.1, and the first of them is the undeployed functions.
