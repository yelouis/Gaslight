...
**✅ SECURITY QUEUE CLOSED — August 21, 2026.** All six defects (Issues 96–101) shipped, deployed and **independently verified**: rules and functions both released and confirmed by `check_deploy_fresh.sh`, `npm --prefix functions test` up from 54 to **61**, analyzer warnings *down* from 26 to 22, and every over-reach guard the spec named present in the tests rather than merely titled. Two design docs that still described the vulnerable model were corrected during verification (§2.19). **One spec warning of mine turned out not to apply and is recorded as a correction so nobody fixes a non-problem.**

**Superseded — August 17, 2026. The "Queue Complete" note below refers to the feature queue and is no longer the whole picture.** A whole-repository security review found **six verified defects**, filed as Issues 96–101 above and specced as SEC1–SEC7 in `agent_execution_guide.md`. One is **HIGH** (Issue 97 — seat and host takeover via `joinRoom`); one had its exploit **reproduced end-to-end against the emulator** (Issue 98). Two production deploys are required: rules for Issue 96, functions for 97/98/99/101.
...
**Security remediation SEC1–SEC7 (Issues 96–101) — shipped August 20–21, 2026; independently verified August 21.** Battery re-measured: `flutter analyze lib test` **0 errors** (22 warnings, 194 infos — both *down* from 26/196) · `flutter test` **156/156** · functions build clean · `npm --prefix functions test` **61/61** (was 54, +7 security tests) · `./scripts/check_deploy_fresh.sh` **exit 0**. **Both deploys landed and were confirmed**: 15/15 functions at `2026-08-21T02:56Z` (after the last `functions/src` commit) and the **rules release at `02:33:34Z`**, after the last `firestore.rules` commit — the separate rules deploy that `predeploy` does not gate was not forgotten.

* **Issue 96 — `/rooms` collection enumeration (`570c613`)**: `firestore.rules` now splits the verbs — `allow get: if true; allow list: if false;` — with the `players` rule left intact because the lobby roster genuinely lists it. **Two** tests in `functions/test/rules.spec.ts` cover authenticated and unauthenticated callers, each with both over-reach guards (room `get` still succeeds; players `list` still succeeds), and the falsification run is recorded verbatim in a comment at `rules.spec.ts:33-34`.
* **Issue 97 — seat and host takeover (`e12fd08`)**: the re-bind now requires **ownership, a valid `seatToken`, or a seat stale >30 s**. The token is minted with `randomUUID()` at `index.ts:133` (createRoom) and `:219` (joinRoom's new-player branch), returned to the caller once, and **only its SHA-256 hash is stored, in `sealed/seat_{playerId}`** (`:139`, `:243`) — the default-deny subcollection. Verified independently: **`grep` finds no `seatToken` in any player-document write**, the rejection throws before the `return { role }` so a failed attempt discloses nothing, `existing.lastSeen ?? 0` treats a legacy document as stale, and every `transaction.get` precedes the write. Client persists per room as `seat_token_{roomCode}` (`game_service.dart:193,231`) and presents it on rejoin (`:222,:260`). The test uses `expect.fail` on a successful takeover so it cannot pass silently, and carries **all four** over-reach guards plus the `role`-absent assertion.
* **Issue 98 — `castVote` answer-key oracle (`f7a7044`)**: `votes` now stores the **opaque option UUID**; the `answerAuthors` lookup is retained only to validate the option and enforce the self-vote guard. **All three missing guards are present** — phase, `currentReaderId`, and already-voted — each throwing `failed-precondition`. Resolution to authors happens server-side at the reveal transition and in `submitUnmaskGuess`. **The `votes` contract was redefined for the third time and this time the readers were walked**: `design_game_state_and_models.md` §2 now documents the two-phase contract and keeps the "broken twice" warning.
* **Issue 99 — reveal merge scoping (`0caba64`)**: sealed content is merged **only** for `room.currentReaderId`'s card; every other card is pushed with `truthAnswer: ""` and `sabotageAnswers: {}`. Test at `game_e2e.spec.ts:461` walks through to card 3. **The half I flagged as most likely to be missed turned out not to apply** — see the correction below.
* **Issue 100 — unmask authorship withheld (`acca606`)**: while `unmaskDeadline` is open, `sabotageAnswers` stays empty and votes stay opaque on the public document; `submitUnmaskGuess` evaluates against the private sealed data. Published only once all fooled players have guessed or the deadline expires. Recorded in `design_scoring_and_ui.md`.
* **Issue 101 — debug callables (`b9fc7e6`)**: **both** guards shipped, not either — `process.env.FUNCTIONS_EMULATOR !== "true"` rejects any production invocation, **and** the caller must be a room member with `isHost`. Recorded in `design_database_and_security.md` §7.1.

**Correction to my own spec.** The guide warned that `advanceToNextResolution` would need to merge the next card's sealed data, or reveals 2 and 3 would render blank. **That concern does not apply.** The next-card branch sets `currentPhase: "vote"`, not `"reveal"` — so each card returns to voting and its sealed content is merged by the ordinary vote→reveal transition, which already scopes to the current reader. The implementation is right and my spec was over-cautious; recorded so nobody "fixes" a non-problem.

**Design gaps found and closed during this verification.** The code was correct but two documents still described the vulnerable model: `design_database_and_security.md` §3 still read *"Room documents: `allow read: if true`"* — the retired rule an agent could have "simplified" back to — and the seat-token mechanism was **entirely undocumented** despite being the HIGH-severity fix. Both corrected, plus the `joinRoom` row of the §2 callable table, which still advertised re-binding on a bare `playerId`. See lesson §2.19.
...
* **Issue 101 (SEC6)** — Gated `debugAddBots` and `debugSimulateBotResponses` in `functions/src/index.ts` on local emulator environment (`process.env.FUNCTIONS_EMULATOR === "true"`, rejecting in production with `permission-denied`) and added host authorization checks requiring caller `authUid` to match a player in the room with `isHost === true`. Verified in `functions/test/game_e2e.spec.ts` with falsification (asserting strangers and non-host members receive `PERMISSION_DENIED`) and over-reach (confirming host caller successfully adds 9 bots and simulates responses in emulator).
* **Issue 100 (SEC5)** — Withheld unmask forgery authorship and sabotage answers server-side in `advancePhaseInternal` (`functions/src/index.ts`) while the 20-second revenge window (`unmaskDeadline`) is active. During the unmask window, `sabotageAnswers` is empty (`{}`) and forgery votes in `card.votes` remain opaque option UUIDs, preventing clients from discovering forgery authors before submitting unmask guesses. `submitUnmaskGuess` evaluates guesses against the private `sealed` subcollection. When all fooled players guess (or when the unmask timer expires and host advances), the server merges `sabotageAnswers` and resolves forgery votes onto the card. Verified in `functions/test/game_e2e.spec.ts` with falsification (asserting `sabotageAnswers` is empty and votes contain no forger IDs during active `unmaskDeadline`) and over-reach (successful guess grading and revelation after closure).
* **Issue 99 (SEC4)** — Scoped the reveal merge in `advancePhaseInternal` (`functions/src/index.ts`) strictly to the active card being resolved (`card.targetPlayerId === room.currentReaderId`). Unread cards in the round remain completely blank (`truthAnswer: ""`, `sabotageAnswers: {}`), preventing premature disclosure of future cards' answers and authorship in public room snapshots. Verified in `functions/test/game_e2e.spec.ts` with falsification (asserting only active reader card carries answers while unread cards are blank at first reveal) and over-reach (walking full round across card resolutions).
* **Issue 98 (SEC3)** — Updated `castVote` in `functions/src/index.ts` to store opaque option UUIDs directly in `card.votes[voterId]` during the `vote` phase, closing the forgery authorship leak on public room snapshots. Added phase check (`currentPhase === "vote"`), active card reader check (`targetCardId === currentReaderId`), and duplicate vote rejection to `castVote`. At the transition to `reveal`, `advancePhaseInternal` resolves option UUIDs into author player IDs via `sealed/{cardId}.answerAuthors` before computing score deltas and merging cards. Verified in `functions/test/game_e2e.spec.ts` with falsification (asserting option ID is stored and author ID is concealed during vote phase), 3 precondition failure guards, and over-reach reveal resolution.
...
* **Issue 88 (Option B)** — 88.1: Client SnackBar exhaustion test added in `test/phase2_craft_test.dart:212–295` asserting `"No more prompts left in this deck."` on `resource-exhausted`; verified live Marionette re-roll sequence on P1; reconciled A4 and repointed dangling reference in `docs/playthrough_findings_marionette.md`. 88.2: `isTimerDisabled: true` test cases added to `test/in_game_leave_test.dart:210–330` across all three in-game phase screens.
...
0e. **Issue 88: Deck exhaustion SnackBar verification and timer-disabled leave controls (Resolved — August 16, 2026)**:
   - **Problem**: (1) A4 in `docs/playthrough_findings_marionette.md` contained a dangling forward reference to A20, leaving the client SnackBar display unverified by automated test. (2) Spec called for testing leave control visibility when `isTimerDisabled: true` on all phase screens.
   - **Solution**: Option B. Added widget tests in `test/in_game_leave_test.dart` for `Phase2CraftScreen`, `Phase3VoteScreen`, and `Phase4RevealScreen` under `isTimerDisabled: true`. Added widget test in `test/phase2_craft_test.dart` asserting `"No more prompts left in this deck."` error SnackBar when `rerollPrompt` throws `resource-exhausted`. Reconciled and updated assertion A4 in `docs/playthrough_findings_marionette.md`.
   - **Verification**: `flutter test test/in_game_leave_test.dart` (6/6 passing) and `flutter test test/phase2_craft_test.dart` (4/4 passing). Full test battery passing 141/141.
...
### 2.15 A test can satisfy a spec's words while testing nothing
