...
55. **Issue 27: Parlor House Rules Consolidation — Option A (Resolved - August 06)**:
...
57. **Issue 29: Icon Dependency Vendored to Eliminate Third-Party Risk & Unused Font Weights — Option B (Resolved - August 06)**:
    ...
    - **Independently verified August 6 — including the two checks the spec flagged as invisible to tests.**
      - **Glyph integrity, proven exhaustively rather than spot-checked.** A wrong code point renders a blank box that every assertion passes, so the font's own `cmap` table was parsed directly and every one of the **11/11 code points confirmed to map to a real glyph** — in both the vendored source (`assets/fonts/phosphor/Phosphor-Light.ttf`, 1543 mapped code points) and the shipped subsetted copy (**exactly 11** mapped, no more). None can render as tofu. Five were additionally confirmed rendering correctly on the iPhone 17 simulator: `writing` (quill, name field), `secret` (key, room-code field), `ledger` (open book, READ MANUAL), `sound` (bell, Parlor AppBar), `envelope` (room-code plaque).
      - **Size, measured — this is the number the spec required and the entry above omitted.** A clean release build gives `Runner.app` **44.0 MB**, against the **46.7 MB** baseline recorded before the work: **2.7 MB / 5.8% smaller**, combining Issue 29's Phosphor weights and T2's `cupertino_icons`. Predicted 2.68 MB; actual 2.7 MB.
      - **Bundle assertions:** exactly **1** Phosphor font ships, subsetted from 536 KB to **8 KB** — proving `--tree-shake-icons` still works on the vendored asset because the map stayed `const`. Both `Phosphor-Light.ttf` and `LICENSE` are tracked by git (`git ls-files`), so the font cannot silently vanish on a fresh clone. Table matches the spec byte for byte: all 11 code points, `fontPackage` omitted, comments retained.
...
58. **Issue 30: Family-Friendly Deck Toggle Hidden from Non-Hosts — Option A (Resolved - August 06)**:
    ...
    - **Independently verified August 6 — falsifiability proven empirically.** Re-run against the pre-change `lobby_screen.dart` (commit `da7278e`), the host-only test **failed** with `Expected: no matching candidates / Actual: Found 1 widget with text "Family-Friendly Decks Only"`. It passes on the current tree. Source confirms `if (isHost)` now guards the control *outside* the `IgnorePointer` block, and it remains client-local — no `updateLobbySettings` call was introduced, so Option C was correctly not implemented by accident. The over-reach guard (genuine house rules still visible to non-hosts) passes against **both** trees, which is what it is for: it would only fail if someone hid the whole card, silently undoing Issue 27.
...
59. **Task T1: Closed Issue 27 Regression Test Coverage Gap (Resolved - August 06)**:
    ...
    - **Independently verified August 6.** File rename confirmed via `git mv` (history follows). All **9** cases present — the original 4 plus 3 from Issue 30 and 2 from T1. The `setupRoomAndPump` helper gained the optional `double textScale = 1.0` parameter exactly as specified, so the 360×640 guard runs at both **1.0 and 1.3** through a single pump path and the four pre-existing cases were unaffected. Client suite rose 60 → **65**.
...
60. **Task T2: Removed Unused `cupertino_icons` Dependency (Resolved - August 06)**:
    ...
    - **Independently verified August 6 — the bundle assertion the spec required.** `find build/ios/iphoneos/Runner.app -iname "CupertinoIcons.ttf"` returns **nothing** on a clean release build; the 252 KB asset is genuinely gone, not merely undeclared. `MaterialIcons-Regular.otf` is still present at **4 KB** (tree-shaken, still referenced by Flutter's own widgets) — the distinction the spec warned not to collapse. The audit greps were re-confirmed to return zero hits. A full app launch on the iPhone 17 simulator showed no missing-glyph fallback anywhere, which is the only way a stray Cupertino reference would have surfaced.

---

61. **Issue 31: Lobby Settings Wipe & START GAME Crash — Option A (Resolved - August 07)**:
...
>
> *Current Queue Status (August 6, 2026):* Issues 23–28 delivered in full with dedicated test coverage. **Final battery: `flutter analyze` 0 errors · `flutter test` 54/54 pass · emulator suite 28/28 pass · TypeScript functions build clean · iOS simulator debug build succeeded.**
