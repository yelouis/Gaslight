...
- **AA12 — ✅ VERIFIED and RESOLVED.** Added in-game manual button and rules guidance in lobby and in-game screens.
- **AA13 — ✅ VERIFIED and RESOLVED.** Rendered final standings above honors on game over screen.
...
**Status**: ✅ Resolved (September 11, 2026) — Implemented Treatment 3 (Stacked Deck with Peek) in `lib/widgets/card_grid.dart` with bidirectional navigation.
- **Problem**: In portrait, `CardGrid` rendered one option per row at a hard-coded `height: 92` inside `BoxConstraints(minHeight: 72, maxHeight: 132)` (`lib/widgets/card_grid.dart`), inside the vote screen's outer `SingleChildScrollView`. Options numbered up to 6 with ~300 logical pixels of chrome before the first option, exceeding typical phone heights and requiring vertical scrolling to read and compare choices.
- **Solution**: Implemented Treatment 3 (Stacked Deck with Peek) with bidirectional navigation. The active card sits in the foreground with subsequent cards peeking out from behind with atmospheric rotation and border accents. Players can navigate bidirectionally using horizontal swipe drag gestures, PREV/NEXT buttons, and interactive jump dots, fully fitting within a 320x640pt viewport without vertical scrolling while preserving 100-character legal answer readability (`AutoSizedAnswerText`). Verified via `test/stacked_deck_navigation_test.dart` and `test/vote_option_truncation_test.dart`.
...
Your selection: Treatment 3 (Stacked Deck with Peek), with ability to navigate/swipe backwards to review and select previously peeled cards.
