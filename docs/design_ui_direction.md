...
- **Re-Roll Cap & Candidate Chooser — shipped Wave AC4, September 2026:** The re-roll prompt button displays `RE-ROLL PROMPT ({n} LEFT)`, starting at `kMaxRerollsPerRound = 3` and decrementing with each re-roll. At 0 remaining, the button disables. If candidates were produced (`_rerollCandidates.isNotEmpty`), a `SELECT A PROMPT` parchment container renders below the re-roll button displaying the distinct candidate prompts rolled this round. The currently selected prompt is marked with an active checkmark. Tapping a candidate selects it via `selectRerolledPrompt`. Collision errors (`failed-precondition`) display a dedicated notice (`Prompt was chosen by another player.`) and remove the candidate from local state. The button label is wrapped in `FittedBox(fit: BoxFit.scaleDown)` to guarantee zero RenderFlex overflow on narrow 320 pt viewports and when virtual keyboards are visible. Covered by `test/phase2_craft_reroll_test.dart`.
...
- **Score Breakdown Collapse & Legibility — shipped Wave AC1 (Issue 171, Option C), September 2026:** In `POINTS AWARDED THIS CARD`, player chips render collapsed by default showing `avatar · {name}: {±N}` so every chip has uniform height regardless of rule count. Tapping a player chip toggles expansion of their itemised rule lines. Expanded rule names render in `AppColors.brass` and point values in `AppColors.ivory` (never `theme.colorScheme.onSurface`, which is `AppColors.ink` and measures 1.12:1 against ground). A verbatim hint `Tap a player to see their score breakdown` in `AppColors.brass` 11pt sits beneath the section heading. Per-player expansion state resets on card advance keyed on `currentReaderId`.
