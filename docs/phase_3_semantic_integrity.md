...
- **API Key Exposure on Client:** The `GEMINI_API_KEY` is loaded via `flutter_dotenv` and sent directly from the Dart HTTP client in `semantic_filter.dart:62`. The key is bundled into the app's `.env` file and visible in network traffic. For prototyping this is acceptable, but for any public release the key must be proxied through a backend or Firebase Cloud Function to prevent unauthorized usage and billing abuse.
- **Race Condition on Concurrent Saboteur Submissions:** Two saboteurs submitting at nearly the same time both call `isAnswerUnique` against the **same snapshot** of `existingAnswers`. Both could pass the `>0.85` check independently and both write near-identical answers to the card, completely defeating the purpose of the deduplication filter. A Firestore transaction or server-side sequential check would be needed to fully prevent this.
