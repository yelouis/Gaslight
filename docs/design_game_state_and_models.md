...
* `votes` (Map<String, String>): A map of `voterPlayerId` to chosen option / author identity. During the `vote` phase, `card.votes[voterId]` stores the **opaque option UUID** to prevent author leaks on public room snapshots. At the transition to `reveal`, `advancePhaseInternal` resolves each option UUID into its author's `playerId` via `sealed/{cardId}.answerAuthors` and writes the resolved IDs into the merged cards for scoring, reveal UI, and unmask guessing.
