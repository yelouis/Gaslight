...
* **Target Forgery Author Guessing (Issue 162 / AA16a / AB1)**: During the vote phase on their own card, the target player may attribute each forgery to a guessed author via `submitTargetForgeryGuesses({ roomCode, cardId, guesses })`. Correct guess: `+1` point (`kTargetForgeryGuessPoints = 1`) awarded to the target for each correctly attributed forgery. Forgers are **not** penalised (unlike revenge guesses, the target was never deceived by the forgery). Guesses are sealed in `sealed/{cardId}.targetForgeryGuesses` during voting and scored via `calculateScoresAndBreakdown` at the vote→reveal transition. Points are exempt from round multipliers (Issue 170 / Option B): guess points pay a flat rate rather than compounding with the round, following the precedent of the unmask revenge ±1 so both "guess a person" rules are consistent. Guess points inherit the unmask score withholding contract: while `unmaskDeadline != null`, guess points are withheld from public `scoreDeltas` in `pendingScoreDeltas` to avoid leaking correct guesses prior to the author flip. Rule id: `target_forger_guess`.
...
* **Points Delta & Unmask Isolation** (Issue 113, Issue 124 & Issue 133, August 2026): Server publishes authoritative `scoreDeltas` on `card` during single-card reveal. When an unmask window opens (`unmaskDeadline != null`), `scoreDeltas` is **withheld** (omitted) and player `totalScore` increments are stashed in `sealed/{cardId}.pendingScoreDeltas` to ensure no forgery authorship or deceiver identity is leaked via delta maps or score changes. Once the unmask window closes (via `submitUnmaskGuess` all-fooled resolution, `closeUnmaskWindow` timeout callable, or reveal advance), the final `scoreDeltas` (including unmask revenge ±1 bonuses/penalties) are written to the public card and player `totalScore`s are incremented. Standings badges display positive (`▲+$delta`) and negative (`▼$delta`) adjustments without client-side recomputation. Unrevealed cards omit `scoreDeltas`.
  * **Itemised Score Breakdown (Issue 169, September 2026)**: Alongside `scoreDeltas`, `ScoringLogic` calculates an itemised `scoreBreakdown` (`Record<string, ScoreBreakdownItem[]>`). During open unmask windows, `scoreBreakdown` is withheld and stashed in `sealed/{cardId}.pendingScoreBreakdown` (with ±1 items appended for revenge unmasks in `submitUnmaskGuess`). Once the window closes, `scoreBreakdown` flushes to public `card.scoreBreakdown` at all three flush sites (`advancePhaseInternal`, `advanceToNextResolution`, `closeUnmaskWindow`). `Phase4RevealScreen` renders itemised lines per rule under each player while strictly preserving visible player totals (`${player.name}: $prefix${e.value}`).
