...
w("  }");
w("");
w("  /// Draws one prompt from [deckId], preferring anything outside [excludedPrompts].");
w("  /// If no unseen prompts remain in [deckId], attempts to draw an unseen prompt");
w("  /// from the fallback deck before relaxing to [mustAvoid] (Issue 174 / Wave AC5).");
w("  static String drawWithFallbackExcluding(");
w("    String deckId,");
w("    Set<String> excludedPrompts, [");
w("    Set<String> mustAvoid = const {},");
w("  ]) {");
...
w("    if (preferred.isNotEmpty) return pick(preferred);");
w("");
w("    if (deckId != fallbackDeckId) {");
w("      return drawOneExcluding(fallbackDeckId, excludedPrompts, mustAvoid);");
w("    }");
w("");
w("    final relaxed = deck.where((p) => !mustAvoid.contains(p)).toList();");
w("    if (relaxed.isNotEmpty) return pick(relaxed);");
