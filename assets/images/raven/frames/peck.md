# `peck.png` — frame grid constraints

**Pose:** `RavenState.peck` · **A vote is confirmed.**
`phase3_vote.dart:286` — pairs with the wax-seal stamp, the app's signature micro-interaction.
...
| Property | Value |
|---|---|
| Hold duration in game | **180 ms** (`_defaultHold` in `raven_pose_host.dart`) |
| Time per frame | **30 ms** |
| Effective frame rate | **33.3 fps** |

Preview at this rate or you will misjudge the motion:

```bash
POSE=peck; COLS=3; ROWS=2; N=6; FPS=33
...
| 1 | row 0, col 1 | (27,29)–(216,224) |  |
| 2 | row 0, col 2 | (12,51)–(236,236) |  |
| 3 | row 1, col 0 | (15,44)–(231,237) |  |
| 4 | row 1, col 1 | (21,35)–(223,228) |  |
| 5 | row 1, col 2 | (25,31)–(218,225) | returns to rest |

**Union across all frames:** (12,29)–(236,237)

**Headroom before clipping:** left **12 px** · top **29 px** · right **19 px** · bottom **18 px**
...
`scripts/build_sprite_sheets.py` rebuilds sheets from the layer art and **silently destroys hand edits**. All poses are opted out of regeneration; confirm `peck` is still listed under the script's skip set before running it, and keep a copy of your edited sheet regardless.
