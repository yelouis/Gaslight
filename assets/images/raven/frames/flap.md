# `flap.png` — frame grid constraints

**Pose:** `RavenState.flap` · **The winner is announced.**
`game_over_screen.dart:45` — the celebration beat.
...
| Property | Value |
|---|---|
| Hold duration in game | **660 ms** (`_defaultHold` in `raven_pose_host.dart`) |
| Time per frame | **110 ms** |
| Effective frame rate | **9.1 fps** |

Preview at this rate or you will misjudge the motion:

```bash
POSE=flap; COLS=3; ROWS=2; N=6; FPS=9
...
| 1 | row 0, col 1 | (29,11)–(215,217) |  |
| 2 | row 0, col 2 | (21,15)–(222,201) |  |
| 3 | row 1, col 0 | (29,1)–(215,207) |  |
| 4 | row 1, col 1 | (21,25)–(222,211) |  |
| 5 | row 1, col 2 | (25,31)–(218,225) | returns to rest |

**Union across all frames:** (21,1)–(222,225)

**Headroom before clipping:** left **21 px** · top **1 px** · right **33 px** · bottom **30 px**

> ⚠️ **This sheet is already tight on the top edge** (top 1px). There is almost no room to make the bird bigger or move it further that way — it will clip.
...
`scripts/build_sprite_sheets.py` rebuilds sheets from the layer art and **silently destroys hand edits**. All poses are opted out of regeneration; confirm `flap` is still listed under the script's skip set before running it, and keep a copy of your edited sheet regardless.
